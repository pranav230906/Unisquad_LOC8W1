...
export default function OTPVerification() {
    ...
    const handleResend = async () => {
        try {
            const { sendOtp } = await import("../../services/authService.js");
            const result = await sendOtp({ phone });
            if (result.dev_otp) {
                showToast(`[DEV] Your new OTP is: ${result.dev_otp}`, "info", 15000);
            } else {
            ...
            <div className="w-full max-w-md animate-in flex flex-col h-full">
                ...
                <div className="space-y-6 mb-4">
                    ...
                    <div className="text-center">
                        <button onClick={handleResend} className="text-[14px] font-semibold text-[#6B7280] hover:text-[#1E3A8A] transition-colors">
                            Resend Code
                        </button>
                    </div>
                </div>
