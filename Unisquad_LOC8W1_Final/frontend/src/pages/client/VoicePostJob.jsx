        ...
        try {
            ...
            await startSpeechToText({
                ...
                onFinal: (t) => {
                    ...
                    // Basic NLP extraction for demo
                    const ltext = t.toLowerCase();

                    // 1. Skill extract
                    if (ltext.includes("कारपेंटर") || ltext.includes("carpenter")) setSkill("Carpenter");
                    else if (ltext.includes("प्लंबर") || ltext.includes("plumber")) setSkill("Plumber");
                    else if (ltext.includes("इलेक्ट्रीशियन") || ltext.includes("electrician")) setSkill("Electrician");
                    else if (ltext.includes("पेंटर") || ltext.includes("painter")) setSkill("Painter");
                    else if (ltext.includes("क्लीनर") || ltext.includes("cleaner")) setSkill("Cleaner");

                    // 2. Radius extract (e.g. "2 किमी" or "2 km")
                    const rMatch = ltext.match(/(\d+)\s*(किमी|km|किलोमीटर)/);
                    if (rMatch) setRadiusKm(Number(rMatch[1]));

                    // 3. When extract (e.g. "कल 5:00")
                    const whenMatch = t.match(/(आज|कल|परसों|today|tomorrow)\s*(\d+:\d+)?/i);
                    if (whenMatch) setWhenText(whenMatch[0]);
                    else if (ltext.includes("कल")) setWhenText("Tomorrow");
                    else if (ltext.includes("आज")) setWhenText("Today");

                    // 4. Address extract (simple fallback)
                    if (t.includes("आजाद नगर")) setAddress("Azad Nagar");
                    else {
                        const words = t.split(' ');
                        const nIdx = words.findIndex(w => w.includes("नगर") || w.includes("Nagar"));
                        if (nIdx > 0) setAddress(words[nIdx - 1] + " Nagar");
                        else setAddress("Extracted from voice");
                    }
                },
            });
        } catch (e) {
    ...
    async function onFindWorkers() {
        try {
            const job = await createJob({
                title: `${skill} Task - ${whenText}`,
                description: finalText ? `Voice transcribed note: ${finalText}` : `Need a ${skill} at ${address}`,
                category: skill.toLowerCase(),
                location: address || "Parsed from voice",
                budget: 500, // Safe default since voice didn't mention it
            });
